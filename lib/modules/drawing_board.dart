...
// import 'package:image/src/draw/draw_line.dart' as draw;
...
class SignatureController extends ValueNotifier<List<Point>> {
  ...
  Future<ui.Image?> toImageWithSize(Size size) async {
    if (isEmpty) {
      return null;
    }
    ...
    SignaturePainter(this).paint(canvas, size);
    final ui.Picture picture = recorder.endRecording();
  ...
  Future<Uint8List?> toPngBytes(Size? size) async {
    if (!kIsWeb) {
      final ui.Image? image =
          size != null ? await toImageWithSize(size) : await toImage();
      if (image == null) {
        return null;
      }
  ...
  Uint8List? _toPngBytesForWeb() {
    ...
    final img.ColorInt32 pColor = img.ColorInt32.rgb(
      penColor.red,
      penColor.green,
      penColor.blue,
    );
    ...
    final img.ColorInt32 bColor = img.ColorInt32.rgba(
    ...
    final img.Image signatureImage =
        img.Image.fromResized(img.Image.empty(), width: width, height: height);
